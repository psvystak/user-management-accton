...
export const rules = {
  avatar: {
    maxLength: helpers.withMessage('999 - це максимум', maxLength(31)),
  ...
  name: {
    required: helpers.withMessage("Поле ім'я обов'язкове", required),
    ...
    noRussian: helpers.withMessage('Дякувати Богу, що я не москаль', helpers.regex(/^[^ЫыЁёЪъ]+$/)),
    noDigits: helpers.withMessage("Ім'я не може містити цифри", helpers.regex(/^[^0-9]+$/)),
    maxLength: helpers.withMessage('Максимальна кілкість символів - 23', maxLength(23)),
  },
  email: {
    required: helpers.withMessage("Поле Емейл адреса обов'язкове", required),
    email: helpers.withMessage('Невірний формат Емейл адреси', email),
    maxLength: helpers.withMessage('Максимальна кілкість символів - 23', maxLength(46)),
  },
  department: {
    required: helpers.withMessage("Поле Департамент обов'язкове", required),
    maxLength: helpers.withMessage('Максимальна кілкість символів - 23', maxLength(23)),
    noConsecutiveSpaces: helpers.withMessage(
